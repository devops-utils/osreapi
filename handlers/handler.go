...
// @Tags Exec
// @Param sort query string false "sort by [start,expired,completed]"
...
func List(c *gin.Context) {
	...
	case "start":
		_res = cache.Cache.GetAllByStartTimes()
	case "expired":
		_res = cache.Cache.GetAllByExpiredTimes()
	case "completed":
		_res = cache.Cache.GetAllByEndTimes()
	default:
	...
	for _, v := range _res {
		...
		if v.ExpiredTimes != 0 {
			_res.Times.Expired = time.Unix(v.ExpiredTimes, 0).Format(time.RFC3339)
		}
		res = append(res, _res)
	}
...
func Post(c *gin.Context) {
	...
	go engine.Process(taskID, jobs, totalTimeOut)
